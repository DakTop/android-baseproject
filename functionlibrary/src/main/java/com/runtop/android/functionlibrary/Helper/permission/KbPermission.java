...
public class KbPermission {
    ...
    public void send() {
        ...
        // 判断是否授权
        if (KbPermissionHelper.getInstance().checkPermission(instance.getWeakActivity().get(), instance.getPermissions())) {
            // 已经授权，执行授权回调
            instance.getListener().onPermit(instance.getRequestCode(), instance.getPermissions());
        } else {
            KbPermissionHelper.getInstance().requestPermission(instance.getWeakActivity().get(), instance.getRequestCode(), instance.getPermissions());
        }
